...
void FIFOScheHandler::handleschedule()
{
    ...
    if (str_nodestate != "")
    {
        ...
        {
            if (debug == 1)
                *debug_file << "Scheduler ---> FIFOScheHandler handleschedule(): Require nodestate = " << endl
                            << nodestate.dump() << endl;
            else if (debug == 2)
    ...
    //test runqueuestate
    Message req_runqueuestate;
    req_runqueuestate.encode_Header("scheduler", "server", "runqueuestate");
    socket->send(req_runqueuestate.encode_Message());
    string str_runqueuestate = socket->receive();
    json json_runqueuestate = json::parse(str_runqueuestate);
    if (debug > 0)
    {
        if (debug == 1)
            *debug_file << "Scheduler ---> FIFOScheHandler handleschedule(): Require runqueuestate = " << endl
                        << json_runqueuestate.dump() << endl;
        else if (debug == 2)
            cout << "Scheduler ---> FIFOScheHandler handleschedule(): Require runqueuestate = " << endl
                 << json_runqueuestate.dump() << endl;
    }
    ...
    if (str_queuestate != "")
    {
        ...
        {
            if (debug == 1)
                *debug_file << "Scheduler ---> FIFOScheHandler handleschedule(): Require queuestate = " << endl
                            << queuestate.dump() << endl;
            else if (debug == 2)
                cout << "Scheduler ---> FIFOScheHandler handleschedule(): Require queuestate = " << endl
                     << queuestate.dump() << endl;
        }
    }
        ...
        if (debug > 0)
        {
            if (debug == 1)
                *debug_file << "Scheduler ---> FIFOScheHandler handleschedule(): End Scheduling." << endl;
                ...
                cout << "Scheduler ---> FIFOScheHandler handleschedule(): End Scheduling. result =" << endl
                     << req_runjob.encode_Message() << endl;
        }
    }
